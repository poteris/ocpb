        ...
        {Array.from({ length: messageCount }).map((_, index) => (
          <div key={index} className={`flex ${index % 2 === 0 ? "justify-start" : "justify-end"}`}>
            <div className={`relative ${index % 2 === 0 ? "pl-2" : ""} max-w-xs`}>
              {index % 2 === 0 && <Skeleton className="absolute -left-3 -bottom-1 w-4 h-4 rounded-full" />}
              <Skeleton
                className={`h-12 w-64 rounded-lg ${index % 2 === 0 ? "bg-pcsprimary01-light" : "bg-pcsprimary-02"}`}
              />
            </div>
          </div>
        ))}
      ...
      <div className="min-h-full p-4">
            ...
            <div className={`relative ${message.sender === "bot" ? "pl-2" : ""}`}>
              <div
                className={`relative max-w-xs px-4 py-2 rounded-lg ${
                  message.sender === "user" ? "bg-slate-50 text-black" : "bg-primary text-white"
                }`}>
                {message.text}
                <div
                  ...
                  <div
                    ...
                    } w-4 h-4 ${message.sender === "user" ? "bg-pcsprimary-02" : "bg-pcsprimary01-light"}`}
                    style={{
                      bottom: "50%",
        ...
        {(isLoading || isWaitingForInitialResponse) && (
          <div key="loading-indicator" className="flex justify-start mb-4">
            <Image width={16} height={16} className="rounded-full mr-2" alt="Bot Avatar" src="/images/bot-avatar.svg" />
