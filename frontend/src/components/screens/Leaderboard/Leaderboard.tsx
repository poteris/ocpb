...
async function fetchLeaderboard(scenarioId: string): Promise<LeaderboardEntry[]> {
  const response = await axios.get(`/api/leaderboard?scenarioId=${scenarioId}`);
  return leaderboardResponseSchema.parse(response.data).entries;
}
...
export const Leaderboard: React.FC<LeaderboardProps> = ({ scenarioId }) => {
  ...
  const loadLeaderboard = useCallback(async () => {
    ...
    try {
      setEntries(await fetchLeaderboard(scenarioId));
    } catch (error) {
